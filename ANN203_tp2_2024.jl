...
\right]
```
where $l_1\in \mathbb{R}^{(n-1)\times 1}$, $B\in \mathbb{R}^{1\times (n-1)}$ and $C\in \mathbb{R}^{(n-1)\times (n-1)}$. We choose $P$ as
```math
P = \left[\begin{matrix}
1 & 0 \\
0 & V_1
\end{matrix}\right] 
```
where $V_1\in \mathbb{R}^{(n-1)\times (n-1)}$ is the Householder matrix of `A[2:n,1]`. It can be easily verified that $P$ is unitary because $V_1$ is unitary. And we have 
```math
PAP^{*} = \left[ \begin{matrix}
a_{11} & BV_1^* \\
V_1 l_1 & V_1 C V_1^*
\end{matrix}
\right]
```
where $V_1 l_1 = [-\text{sign}(a_{21})||l_1||,0,...,0]^{T}$.
"""
)

# ╔═╡ 075d8611-1628-45d5-b69e-79e383536ef0
answer(md"""
**(First step $k=1$)**: We search $q_2$ such that $q_1^{H}q_2 = 0$ and $||q_2||=1$. Therefore we have
```math
Aq_1 = h_{11}q_1 + h_{21}q_2
```
which leads to $h_{11} = q_1^{H}Aq_1, h_{21} = ||Aq_1 - h_{11}q_1||$ and $q_2 = (Aq_1 - h_{11}q_1)/h_{21}$.

**(Recursive steps for $k\geq2$)**: We assume that we have found $q_1, q_2,...,q_{k}$ and have found $h_{ji},\forall 1\leq j \leq k$, $\forall 1\leq i \leq k-1$. Then, according to the relation $AQ_k = Q_{k+1} H_k$, we have 
```math
Aq_k = h_{1k}q_1 + h_{2k}q_2 + ... + h_{k+1,k}q_{k+1}
```
where $q_1,q_2,...,q_{k+1}$ are orthonormal. This leads to $\forall 1\leq j \leq k$, 
```math
h_{jk} = q_j^{H} A q_k\quad \text{and} \quad h_{k+1,k} = ||Aq_k - h_{1k}q_1 - ... - h_{kk}q_k||
```
and
```math
q_{k+1} = \frac{Aq_k - h_{1k}q_1 - ... - h_{kk}q_k}{h_{k+1,k}}
```
""")
...
manifest_format = "2.0"
project_hash = "cc792d278927c15f00a81f94a3c78a45dc119c27"
